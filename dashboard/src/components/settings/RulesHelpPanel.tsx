  ...
  return (
    <div className="border-b border-border/50 last:border-0">
      <button
        ...
        className="w-full flex items-center gap-2 px-3 py-2.5 text-sm font-medium text-foreground hover:bg-muted/50 transition-colors text-left"
      >
        <Icon className="h-3.5 w-3.5 text-muted-foreground/60 shrink-0" />
        <span className="flex-1">{title}</span>
        <ChevronRight className={cn('h-3.5 w-3.5 text-muted-foreground/60 shrink-0 transition-transform', open && 'rotate-90')} />
      </button>
  ...
  return (
    <div className="h-full overflow-auto bg-white border border-border rounded-lg">
      <div className="px-3 py-3 border-b border-border/50 flex items-center gap-2">
        ...
        <span className="text-sm font-semibold text-foreground">Rule Reference</span>
      </div>
    ...
    severity: high`}
      ...
      <Section title="Available Fields" icon={Hash}>
        <div className="space-y-1.5 text-xs text-muted-foreground">
          <p className="text-muted-foreground mb-1">Fields you can match against on each event:</p>
          {[
      ...
      <Section title="Operators" icon={Gauge}>
        <div className="space-y-1.5 text-xs text-muted-foreground">
          <p className="text-muted-foreground mb-1">Comparison operators for simple conditions:</p>
          {[
            ...
            <div key={o.op} className="flex gap-2">
              <code className="shrink-0 font-mono text-[11px] bg-muted px-1 py-0.5 rounded text-foreground w-20 text-center">{o.op}</code>
      ...
      <Section title="Composite Conditions" icon={Braces}>
        <div className="space-y-2 text-xs text-muted-foreground">
          <p>Combine conditions with <code className="font-mono bg-muted px-1 rounded">and</code> or <code className="font-mono bg-muted px-1 rounded">or</code>:</p>
          <pre className="bg-muted/50 rounded p-2 font-mono text-[11px] text-foreground overflow-x-auto">
{`condition:
  and:
    - field: type
      ...
      <Section title="Severity Levels" icon={AlertTriangle}>
        <div className="space-y-1 text-xs text-muted-foreground">
          {[
            ...
            { level: 'low', color: 'text-muted-foreground', desc: 'Informational' },
          ].map((s) => (
